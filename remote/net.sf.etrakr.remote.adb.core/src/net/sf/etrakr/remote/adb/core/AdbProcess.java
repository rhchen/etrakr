...
import net.sf.etrakr.remote.adb.core.adb.AdbChannel;
...
public class AdbProcess implements IRemoteProcessControlService, IRemoteProcessSignalService, IRemoteProcessTerminalService {
	...
	private final AdbChannel fChannel;
	...
	public void setTerminalSize(int cols, int rows, int width, int height) {
		if (fChannel instanceof AdbChannelExec) {
			((AdbChannelExec) fChannel).setPtySize(cols, rows, width, height);
		} else if (fChannel instanceof AdbChannelShell) {
			((AdbChannelShell) fChannel).setPtySize(cols, rows, width, height);
		}
	}
