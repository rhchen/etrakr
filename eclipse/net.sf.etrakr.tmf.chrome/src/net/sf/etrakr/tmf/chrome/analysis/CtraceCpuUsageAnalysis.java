...
import net.sf.etrakr.tmf.chrome.state.CtraceCpuStateProvider;
        ...
        try {
            ...
            for (Entry<Integer, List<Integer>> entry : tidsPerCpu.entrySet()) {
                ...
                for (int tidNode : tidNodes) {
                    ...
                    if (currentCount < 0) {
                    	TmfChromeActivator.getDefault().logWarning(String.format("Negative count: start %d, end %d", countAtStart, countAtEnd)); //$NON-NLS-1$
                        currentCount = 0;
                    } else if (currentCount > endTime - startTime) {
                    	TmfChromeActivator.getDefault().logWarning(String.format("CPU Usage: Spent more time on CPU than allowed: %s spent %d when max should be %d", curTidName, currentCount, endTime - startTime)); //$NON-NLS-1$
                        currentCount = 0;
                    }
             ...
             * should not happen.
             */
        	TmfChromeActivator.getDefault().logError("Error getting CPU usage in a time range", e); //$NON-NLS-1$
