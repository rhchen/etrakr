 ...
 * @since 3.0
 */
public class FtraceCpuUsageAnalysis extends TmfStateSystemAnalysisModule {

    /** The ID of this analysis */
    public static final String ID = "net.sf.etrakr.tmf.ftrace.analysis.ftrace.cpuusage"; //$NON-NLS-1$
    ...
    protected ITmfStateProvider createStateProvider() {
        return new FtraceCpuStateProvider(getTrace());
    }
        ...
        try {
            ...
            for (Entry<Integer, List<Integer>> entry : tidsPerCpu.entrySet()) {
                ...
                for (int tidNode : tidNodes) {
                    ...
                    if (currentCount < 0) {
                    	TmfFtraceActivator.getDefault().logWarning(String.format("Negative count: start %d, end %d", countAtStart, countAtEnd)); //$NON-NLS-1$
                        currentCount = 0;
                    } else if (currentCount > endTime - startTime) {
                    	TmfFtraceActivator.getDefault().logWarning(String.format("CPU Usage: Spent more time on CPU than allowed: %s spent %d when max should be %d", curTidName, currentCount, endTime - startTime)); //$NON-NLS-1$
                        currentCount = 0;
                    }
             ...
             * should not happen.
             */
        	TmfFtraceActivator.getDefault().logError("Error getting CPU usage in a time range", e); //$NON-NLS-1$
