...
package net.sf.etrakr.ctrace.ui.views.cpu;
...
import net.sf.etrakr.tmf.ctrace.analysis.CtraceCpuUsageAnalysis;
...
public class CpuUsageComposite extends AbstractTmfTreeViewer {
    ...
    /* Fix me
     * Should be generic, since there is extension point declaration
     */
    private CtraceCpuUsageAnalysis fModule = null;
    ...
    protected ITmfTreeColumnDataProvider getColumnDataProvider() {
        return new ITmfTreeColumnDataProvider() {
            ...
            public List<TmfTreeColumnData> getColumnData() {
                ...
                List<TmfTreeColumnData> columns = new ArrayList<TmfTreeColumnData>();
    ...
    protected ITmfTreeViewerEntry updateElements(long start, long end, boolean isSelection) {
        ...
        for (Entry<String, Long> entry : cpuUsageMap.entrySet()) {
            ...
            if (!entry.getKey().startsWith(CtraceCpuUsageAnalysis.TOTAL)) {
                continue;
            }
            String[] strings = entry.getKey().split(CtraceCpuUsageAnalysis.SPLIT_STRING, 2);

            if ((strings.length > 1) && !(strings[1].equals(CtraceCpuUsageAnalysis.TID_ZERO))) {
    ...
    private String getProcessName(String tid) {
        ...
        ITmfStateSystem kernelSs = TmfStateSystemAnalysisModule.getStateSystem(trace, CtraceCpuUsageAnalysis.ID);
        if (kernelSs == null) {
            return tid;
        }
