...
package net.sf.etrakr.tmf.ctrace.analysis;
...
import net.sf.etrakr.tmf.ctrace.state.CtraceCpuStateProvider;
 ...
 * @since 3.0
 */
public class CtraceCpuUsageAnalysis extends TmfStateSystemAnalysisModule {

    /** The ID of this analysis */
    public static final String ID = "net.sf.etrakr.tmf.ctrace.analysis.ctrace.cpuusage"; //$NON-NLS-1$
    ...
    protected ITmfStateProvider createStateProvider() {
        return new CtraceCpuStateProvider(getTrace());
    }
    ...
    protected boolean executeAnalysis(IProgressMonitor monitor) {
         ...
         */
    	CtraceAnalysisModule module = TmfTraceUtils.getAnalysisModuleOfClass(getTrace(), CtraceAnalysisModule.class, CtraceAnalysisModule.ID);
        if (module != null) {
            module.schedule();
        }
    ...
    public Map<String, Long> getCpuUsageInRange(long start, long end) {
        ...
        ITmfStateSystem kernelSs = TmfStateSystemAnalysisModule.getStateSystem(trace, CtraceAnalysisModule.ID);
        if (kernelSs == null) {
            return map;
        }
        ...
        try {
            ...
            Map<Integer, List<Integer>> tidsPerCpu = new HashMap<Integer, List<Integer>>();
            ...
            for (Entry<Integer, List<Integer>> entry : tidsPerCpu.entrySet()) {
                ...
                for (int tidNode : tidNodes) {
                    ...
                    if (currentCount < 0) {
                    	TmfCtraceActivator.getDefault().logWarning(String.format("Negative count: start %d, end %d", countAtStart, countAtEnd)); //$NON-NLS-1$
                        currentCount = 0;
                    } else if (currentCount > endTime - startTime) {
                    	TmfCtraceActivator.getDefault().logWarning(String.format("CPU Usage: Spent more time on CPU than allowed: %s spent %d when max should be %d", curTidName, currentCount, endTime - startTime)); //$NON-NLS-1$
                        currentCount = 0;
                    }
             ...
             * should not happen.
             */
        	TmfCtraceActivator.getDefault().logError("Error getting CPU usage in a time range", e); //$NON-NLS-1$
