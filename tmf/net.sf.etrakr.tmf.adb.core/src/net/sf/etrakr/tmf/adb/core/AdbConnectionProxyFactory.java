...
import net.sf.etrakr.tmf.adb.core.internal.AdbSocketFactory;
import net.sf.etrakr.tmf.adb.core.internal.Proxy;
...
public class AdbConnectionProxyFactory {
	private static class CommandProxy implements Proxy {
		 ...
		 */
		@Override
		public void connect(AdbSocketFactory socket_factory, String host, int port, int timeout) throws IOException {
			...
			if (connection != null) {
				...
				process = AdbActivator.getService(IRemoteServicesManager.class).getLocalConnectionType().getConnections().get(0).
			...
			new Thread() {
				@Override
				public void run() {
					final ILog log = AdbActivator.getDefault().getLog();
					String line;
					try {
						while ((line = bufferedReader.readLine()) != null) {
							log.log(new Status(IStatus.INFO, AdbActivator.getUniqueIdentifier(), IStatus.OK, line, null));
						}
					} catch (IOException e) {
						AdbActivator.log(e);
					}
	...
	private static class SSHForwardProxy implements Proxy {
		 ...
		 */
		@Override
		public void connect(AdbSocketFactory socket_factory, String host, int port, int timeout) throws Exception {
			assert !connectCalled : "connect should only be called once"; //$NON-NLS-1$
			try {
