...
public class AdbBuffer {

	final byte[] tmp=new byte[4];
	  byte[] buffer;
	  int index;
	  int s;
	  public AdbBuffer(int size){
	    buffer=new byte[size];
	    index=0;
	    s=0;
	  }
	  public AdbBuffer(byte[] buffer){
	    this.buffer=buffer;
	    index=0;
	    s=0;
	  }
	  public AdbBuffer(){ this(1024*10*2); }
	  ...
	  void checkFreeSize(int n){
	    int size = index+n+AdbSession.buffer_margin;
	  ...
	  byte[][] getBytes(int n, String msg) throws AdbException {
	    byte[][] tmp = new byte[n][];
	    for(int i = 0; i < n; i++){
	      int j = getInt();
	      if(getLength() < j){
	  ...
	  */
	  ...
	  static AdbBuffer fromBytes(byte[][] args){
	    int length = args.length*4;
	    for(int i = 0; i < args.length; i++){
	      length += args[i].length;
	    }
	    AdbBuffer buf = new AdbBuffer(length);
	    for(int i = 0; i < args.length; i++){
	      buf.putString(args[i]);
	    }
	    return buf;
