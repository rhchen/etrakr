...
/*
 * this is a copy from org.eclipse.swt.graphics.Rectangle but with long values
 */
/**
 ...
 */
public final class RectangleLong implements Serializable {

	/**
	 * 
	 */
	private static final long	serialVersionUID	= -7428687765205784135L;
	 ...
	 *            the height of the rectangle
	 */
	public RectangleLong(final long x, final long y, final long width, final long height) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	 ...
	 *                </ul>
	 */
	public void add(final RectangleLong rect) {
		if (rect == null) {
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		}
		final long left = x < rect.x ? x : rect.x;
		final long top = y < rect.y ? y : rect.y;
		long lhs = x + width;
	...
	public boolean equals(final Object object) {
		...
		if (!(object instanceof RectangleLong)) {
			return false;
		}
		final RectangleLong r = (RectangleLong) object;
	 ...
	 *                since 3.0
	 */
	public void intersect(final RectangleLong rect) {
		if (rect == null) {
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		}
		if (this == rect) {
	 ...
	 *                </ul>
	 */
	public RectangleLong intersection(final RectangleLong rect) {
		if (rect == null) {
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		}
		if (this == rect) {
			return new RectangleLong(x, y, width, height);
		}
		final long left = x > rect.x ? x : rect.x;
		final long top = y > rect.y ? y : rect.y;
		long lhs = x + width;
	 ...
	 * @see #isEmpty()
	 */
	public boolean intersects(final RectangleLong rect) {
		if (rect == null) {
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		}
	 ...
	 * @see #add(Rectangle)
	 */
	public RectangleLong union(final RectangleLong rect) {
		if (rect == null) {
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		}
		final long left = x < rect.x ? x : rect.x;
		...
		rhs = rect.y + rect.height;
		final long bottom = lhs > rhs ? lhs : rhs;
		return new RectangleLong(left, top, right - left, bottom - top);
	}
