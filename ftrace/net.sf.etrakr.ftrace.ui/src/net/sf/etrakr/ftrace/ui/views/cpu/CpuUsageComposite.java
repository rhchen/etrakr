...
public class CpuUsageComposite extends AbstractTmfTreeViewer {
     ...
     */
    private FtraceCpuUsageAnalysis fModule = null;
    ...
    public void initializeDataSource() {
        ...
        fModule = TmfTraceUtils.getAnalysisModuleOfClass(trace, FtraceCpuUsageAnalysis.class, FtraceCpuUsageAnalysis.ID);
        if (fModule == null) {
            return;
        }
        fModule.schedule();
    ...
    protected ITmfTreeViewerEntry updateElements(long start, long end, boolean isSelection) {
        ...
        for (Entry<String, Long> entry : cpuUsageMap.entrySet()) {
            ...
            if (!entry.getKey().startsWith(FtraceCpuUsageAnalysis.TOTAL)) {
                continue;
            }
            String[] strings = entry.getKey().split(FtraceCpuUsageAnalysis.SPLIT_STRING, 2);

            if ((strings.length > 1) && !(strings[1].equals(FtraceCpuUsageAnalysis.TID_ZERO))) {
